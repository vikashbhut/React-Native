...
export const GET_HOLDINGS_SUCCESS = "GET_HOLDINGS_SUCCES";
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          let myHoldings = response.data.map((item) => {
            //Retrive our current holding current quentity
...
export const getCoingMarketFailure = (error) => ({
    ...
    return axios({
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          dispatch(getCoingMarketFailure(response.data));
        }
      })
      .catch((error) => {
        dispatch(getCoingMarketFailure(error));
      });
  };
}
